...
def sum_energies():
    ...
    if rank == 0:
        total = local.copy()
        recvbuf = np.empty(2, dtype=np.float64)

        for src in range(1, size):
            comm.Recv(recvbuf, source=src, tag=200)
            total += recvbuf

        return total[0], total[1]

    else:
        comm.Send(local, dest=0, tag=200)
        return None, None
...
def main():
    ...
    for nstep in range(1, NSTEPS + 1):
        ...
        if nstep % WRITE_FREQ == 0:
            pot_total, kin_total = sum_energies()
    ...
    if rank == 0:
        ...
        f.close()
        print(f"PARALLEL POINT-TO-POINT TIME = {end - start:.6f} SECONDS")
