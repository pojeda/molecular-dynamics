...
SUBROUTINE EXCHANGE_COORDINATES()
  ...
  DO SRC = 0, NPROCS-1
     ...
     ELSE
        NLOC = BASE
        S = REM*(BASE+1) + (SRC-REM)*BASE + 1
     ENDIF

     IF (MYRANK == SRC) THEN
        DO DEST = 0, NPROCS-1
           IF (DEST /= SRC) THEN
              CALL MPI_SEND(COOR(1,S), 3*NLOC, MPI_DOUBLE_PRECISION, DEST, 100, MPI_COMM_WORLD, IERR)
           ENDIF
        ENDDO
     ELSE
        CALL MPI_RECV(COOR(1,S), 3*NLOC, MPI_DOUBLE_PRECISION, SRC, 100, MPI_COMM_WORLD, STATUS, IERR)
     ENDIF
...
SUBROUTINE SUM_ENERGIES(POT_TOTAL, KIN_TOTAL)
  ...
  IF (MYRANK == 0) THEN
     ...
     DO SRC = 1, NPROCS-1
        CALL MPI_RECV(BUFFER, 2, MPI_DOUBLE_PRECISION, SRC, 200, MPI_COMM_WORLD, STATUS, IERR)
        POT_TOTAL = POT_TOTAL + BUFFER(1)
        KIN_TOTAL = KIN_TOTAL + BUFFER(2)
     ENDDO
  ...
  ELSE
     ...
     CALL MPI_SEND(BUFFER, 2, MPI_DOUBLE_PRECISION, 0, 200, MPI_COMM_WORLD, IERR)
