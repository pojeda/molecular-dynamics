...
DO I=1,NUM_RES-2
    ...
    DISX=PARTICLE(I)%COORX(N)-PARTICLE(J)%COORX(N)                          
    DISY=PARTICLE(I)%COORY(N)-PARTICLE(J)%COORY(N)                          
    DISZ=PARTICLE(I)%COORZ(N)-PARTICLE(J)%COORZ(N)                           
    DISD=DISX*DISX + DISY*DISY + DISZ*DISZ        


           FDUMMY1=SIGMA_CONST(I,J)**2/DISD
           K=CLASE(I)
           L=CLASE(J)
...
DO I=2,NUM_RES-1
    ...
    DISX=PARTICLE(I)%COORX(N)-PARTICLE(I+1)%COORX(N)                          
    DISY=PARTICLE(I)%COORY(N)-PARTICLE(I+1)%COORY(N)                          
    DISZ=PARTICLE(I)%COORZ(N)-PARTICLE(I+1)%COORZ(N)                           
    DISD=SQRT(DISX*DISX + DISY*DISY + DISZ*DISZ)        

    DISPX=PARTICLE(I)%COORX(N)-PARTICLE(I-1)%COORX(N)                          
    DISPY=PARTICLE(I)%COORY(N)-PARTICLE(I-1)%COORY(N)                          
    DISPZ=PARTICLE(I)%COORZ(N)-PARTICLE(I-1)%COORZ(N)                           
...
ENDDO
    ...
    DISX=PARTICLE(1)%COORX(N)-PARTICLE(2)%COORX(N)                          
    DISY=PARTICLE(1)%COORY(N)-PARTICLE(2)%COORY(N)                          
    DISZ=PARTICLE(1)%COORZ(N)-PARTICLE(2)%COORZ(N)                           
    DISD=SQRT(DISX*DISX + DISY*DISY + DISZ*DISZ)       
      FDUMMY1=DISD-R_CERO
      FDUMMY2=FDUMMY1*FDUMMY1
      ENER2=ENER2+FDUMMY2
    ...
    DISX=PARTICLE(NUM_RES)%COORX(N)-PARTICLE(NUM_RES-1)%COORX(N)                          
    DISY=PARTICLE(NUM_RES)%COORY(N)-PARTICLE(NUM_RES-1)%COORY(N)                          
    DISZ=PARTICLE(NUM_RES)%COORZ(N)-PARTICLE(NUM_RES-1)%COORZ(N)                           
    DISD=SQRT(DISX*DISX + DISY*DISY + DISZ*DISZ)       
      FDUMMY1=DISD-R_CERO
